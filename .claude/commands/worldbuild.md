...
3. **连续短句（铁律八）**: 连续≤15字符短句超过3个（硬科幻风格可放宽至3个，≥4必须修复）
    - 🔴 FORBIDDEN: `韦伯沉默。沉默来自接受。接受来自价值超过损失。价值来自轨道角度。`
...
**生成 prose_style.md 时必须包含以下反机械化规则，防止精度风格过拟合：**

### 感官锚定去重规则
同一感官锚定（嗡鸣、微酸味、冷感等）在全小说中以相同措辞出现不超过3次。每次出现使用不同描述方式（不同角度、不同感知方式、不同角色感知）。

### 数字密度控制规则
数值密度不超过8%句子（每100句中不超过8句含数字/测量值）。关键情绪时刻优先感官描写，数字仅作支持。禁止连续3句以上含精确数值。

### 段落开头多样性规则
任何角色名开头段落不超过12%总段落数。连续2个段落不得以同一角色名开头。穿插动作开头、环境开头、对话开头、时间开头。

### "不是...是..."密度控制规则
"不是...是..."结构每章不超过3次。用正面陈述替代或逗号合并句式。

### 段落长度分布规则
单句段落不超过15%总段落数。每个场景（约500-800字）至少包含1个中长段落（3+句）。混合短、中、长段落制造节奏变化。
